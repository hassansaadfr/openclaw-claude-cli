...
# Install Claude Code CLI and keyring for credential persistence
RUN apt-get update && \
    apt-get install -y --no-install-recommends libsecret-1-0 dbus gnome-keyring && \
    rm -rf /var/lib/apt/lists/* && \
    npm install -g @anthropic-ai/claude-code

# Add billing proxy (zero-dependency Node script)
RUN mkdir -p /opt/billing-proxy
COPY billing-proxy/ /opt/billing-proxy/
...
RUN mkdir -p /data/uv /data/go /data/workspace /data/.openclaw /home/node/.claude /home/node/.local/share/keyrings && \
    chown -R 1000:1000 /data /home/node /opt/billing-proxy
